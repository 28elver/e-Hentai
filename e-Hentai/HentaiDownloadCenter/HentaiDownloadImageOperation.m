...
- (void)start {
	...
	[self hentaiStart];
	NSNumber *imageHeight = HentaiCacheLibraryDictionary[self.hentaiKey][[zNsMeth zMethReturnTrueFileNameOnAddLastPathComponent:self.downloadURLString]];
	...
	if (![self isCancelled]) {
		dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
		    ...
		    if (self.isCacheOperation) {
		        [[[[FilesManager cacheFolder] fcd:@"Hentai"] fcd:self.hentaiKey] write:UIImageJPEGRepresentation(image, 0.6f) filename:[zNsMeth zMethReturnTrueFileNameOnAddLastPathComponent:self.downloadURLString]];
			}
		    else {
		        [[[FilesManager documentFolder] fcd:self.hentaiKey] write:UIImageJPEGRepresentation(image, 0.6f) filename:[zNsMeth zMethReturnTrueFileNameOnAddLastPathComponent:self.downloadURLString]];
			}
